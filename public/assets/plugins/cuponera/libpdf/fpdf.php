...
function __construct($orientation='P',$unit='mm',$format='A4')
{
	...
	$this->CoreFonts=array('courier'=>'Courier','courierB'=>'Courier-Bold','courierI'=>'Courier-Oblique','courierBI'=>'Courier-BoldOblique',
		...
		'symbol'=>'Symbol','zapfdingbats'=>'ZapfDingbats','coure10'=>'coure10');
	//Scale factor
	if($unit=='pt')
		$this->k=1;
	elseif($unit=='mm')
	...
	if(is_string($format))
	{
		...
		elseif($format=='a4')
			$format=array(595.28,841.89);//valor por defecto  (595.28,841.89)
		elseif($format=='a5')
		...
		elseif($format=='continuo')
			$format=array(646.3,790.87);
		elseif($format=='pu')//610.58,856//680.58,840
			$format=array(610.58,857); //para Impresion de PU Agreado por Vhidalgo no borrar pendejos!!!
		elseif($format=='certificado') //Para licencia certificado
			$format=array(637.8,796.54); // 22.5 X 281
                else
...
public function Write($h,$txt,$link='')
{
	...
	//Last chunk
	if($i!=$j)
		$this->Cell($l/1000*$this->FontSize,$h,substr($s,$j),0,0,'J',0,$link);
}

public function Write1($h,$txt,$link='')
{
	//Output text in flowing mode
	$cw=&$this->CurrentFont['cw'];
	$w=$this->w-$this->rMargin-$this->x;
	...
	while($i<$nb)
	{
		...
		if($l>$wmax)
		{
			...
			if($sep==-1)
			{
				...
				if($i==$j)
					$i++;
				$this->Cell($w,$h,substr($s,$j,$i-$j),0,2,'J',0,$link);
			}
			else
			{
				$this->Cell($w,$h,substr($s,$j,$sep-$j),0,2,'J',0,$link);
				$i=$sep+1;
			}
			$sep=-1;
			$j=$i;
...
public function Image($file,$x,$y,$w=0,$h=0,$type='',$link='')
{
	...
	if(!isset($this->images[$file]))
	{
		...
		//$mqr=get_magic_quotes_runtime();		> php 5
...
public function _putfonts()
{
	...
	//set_magic_quotes_runtime($mqr); // no sirve en php < 5.xx
	ini_set("magic_quotes_runtime", 0);
